...
void func2(){
	...
	while(1){
        if(GetAsyncKeyState(VK_ESCAPE)){
        	printf("%s","<Trying to stop the program... >\n");
        	kill = 0;		
    }
}
	...
	if(kill == 1){
		while (1) {
			default_random_engine e;
	   		uniform_real_distribution<double> u(-8.0, 15.0);
			u(e);
			int(u(e));
...
int main()
{
	...
	while (true){
		...
		printf("%s","Program Version 3.5.7(build 3132BC_220710) Release\n");
		printf("%s","Compiler Version TDM-GCC 9.2.0 64-bit Release\n");
		...
		cout << endl;
		cout << endl;
		printf("%s","What's new (compared to the previous version): \n [Version 3.5(Update 20) Made by 12sdj]\n");
		printf("%s","1.Fix one or more bug(s)\n");
		printf("%s","2.Some formulations have been revised, and we deleted some error descriptions\n");
		printf("%s","3.Increase the speed of the program\n");
		printf("%s","4.Improve the overall stability of the program, improve the efficient use of the program CPU and RAM\n");
		cout << endl;
		system("color 02");
		...
		cout << endl;
		printf("%s","Please input CPS[CPS <= 120]: ");
		...
		cout << endl;
		system("cls");
		cout << "MouseClicker Basic" << endl;

		system("color");
		
		cout << endl;
		printf("%s","Checking your pleasure");
		for(int i=1; i<=3; i++){
			printf("%s",".");
			Sleep(400);
		}
		...
		printf("%s","<The main program is creating and splitting processes>\n");
		...
		if(af == 1){
			...
			system("cls");
			printf("%s","<Execution>\n");
			printf("%s","Tips: You need to press ESC to EXIT the program\n");
